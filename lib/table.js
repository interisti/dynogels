...
Table.prototype.runQuery = function (params, callback) {
  ...
  self.sendRequest('query', params, internals.deserializeItems(self, (err, data) => {
    if (err) {
      return callback(err);
    }

    self._after.emit('query', data);
...
Table.prototype.runScan = function (params, callback) {
  ...
  self.sendRequest('scan', params, internals.deserializeItems(self, (err, data) => {
    if (err) {
      return callback(err);
    }

    self._after.emit('scan', data);
    return callback(err, data);
  }));
};
